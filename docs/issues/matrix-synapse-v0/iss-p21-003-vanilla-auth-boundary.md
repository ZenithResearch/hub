# ISS-P21-003: Vanilla auth boundary

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P21-003: Vanilla auth boundary
- **Suggested branch:** `issue/iss-p21-003-vanilla-auth-boundary`
- **Suggested PR title:** `ISS-P21-003: Vanilla auth boundary`
- **Primary repo:** `ZenithResearch/hub`
- **Supporting repo/API dependency:** none identified for this issue note
- **Source vault note:** `private source note: iss-p21-003`
- **GitHub issue:** https://github.com/ZenithResearch/hub/issues/47
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p21-003-vanilla-auth-boundary.md`

## Full spec

### Objective

Keep Matrix-triggered admin/provisioning actions behind existing Hub/Gateway auth and prove Matrix sender identity is not authority.
...
- PRP-PR-018 routing/provenance; existing Hub/Gateway auth semantics

### Target files and surfaces

- Hub dispatch/routes, services/ingest, gateway_http, queue workers, Hub auth/session/provisioning controls

### Locked decisions and invariants

- V0 auth remains vanilla Hub/Gateway auth.
- Matrix sender identity is provenance/context, not Hub authority.
- Admin/provisioning routes must fail closed without existing Hub/Gateway auth.

### Acceptance criteria

- Matrix-originated admin/provisioning request without Hub/Gateway auth denies before action execution.
- Ordinary mention/concierge work still enqueues according to routing config.
- Evidence is recorded in the implementation repo or linked capture before this issue is marked complete.
- The project note is updated with completion evidence and downstream blockers.

### Verification commands

- hub pytest; focused ingest/gateway auth tests; fixture smoke with redacted tokens.
...
**Commit boundary:** one commit in the `ISS-P21-003` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P21-003`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p21-003"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p21-003 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p21-003"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p21-003 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p21-003 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p21-003 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p21-003`.
