# ISS-P14-002: Core Terraform resource adoption

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P14-002: Core Terraform resource adoption
- **Suggested branch:** `issue/iss-p14-002-core-terraform-resource-adoption`
- **Suggested PR title:** `ISS-P14-002: Core Terraform resource adoption`
- **Primary repo:** `ZenithResearch/hub`
- **Supporting repo/API dependency:** none identified for this issue note
- **Source vault note:** `private source note: iss-p14-002`
- **GitHub issue:** https://github.com/ZenithResearch/hub/issues/30
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p14-002-core-terraform-resource-adoption.md`

## Full spec

### Objective

Port or adapt Matrix EC2 + encrypted EBS resources into `infra/aws_baseline_80` as inspectable production Terraform files.
...
- infra/aws_baseline_80/matrix*.tf, matrix_user_data.sh.tpl, variables.tf, outputs.tf
...
- Terraform resources are present in the core prod tree; plan includes intended Synapse resources when enabled; source keeps generated appservice registrations runtime-only.
...
- terraform fmt -check; terraform init/validate; production plan with explicit tags; static resource tests.
...
**Commit boundary:** one commit in the `ISS-P14-002` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P14-002`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p14-002"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p14-002 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p14-002"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p14-002 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p14-002 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p14-002 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p14-002`.
