# ISS-P18-002: Outbound Matrix adapter

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P18-002: Outbound Matrix adapter
- **Suggested branch:** `issue/iss-p18-002-outbound-matrix-adapter`
- **Suggested PR title:** `ISS-P18-002: Outbound Matrix adapter`
- **Primary repo:** `ZenithResearch/hub`
- **Supporting repo/API dependency:** none identified for this issue note
- **Source vault note:** `private source note: iss-p18-002`
- **GitHub issue:** https://github.com/ZenithResearch/hub/issues/41
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p18-002-outbound-matrix-adapter.md`

## Full spec

### Objective

Implement reusable Hub outbound adapter for Matrix messages/replies with traceable delivery result.
...
- ISS-P18-001, PRP-PR-015 token readiness

### Target files and surfaces

- services/matrix_bridge/main.py, gateway Matrix notify code, services/ingest/matrix_client.py, tests
...
- Adapter maps canonical payload to Matrix send call; records Matrix event ID or structured failure; redacts tokens.
...
- pytest adapter tests; mocked Matrix API test; ruff.
...
**Commit boundary:** one commit in the `ISS-P18-002` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P18-002`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p18-002"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p18-002 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p18-002"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p18-002 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p18-002 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p18-002 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p18-002`.
