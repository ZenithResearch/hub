# ISS-P21-004: Ordinary execution path

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P21-004: Ordinary execution path
- **Suggested branch:** `issue/iss-p21-004-ordinary-execution-path`
- **Suggested PR title:** `ISS-P21-004: Ordinary execution path`
- **Primary repo:** `ZenithResearch/hub`
- **Supporting repo/API dependency:** none identified for this issue note
- **Source vault note:** `private source note: iss-p21-004`
- **GitHub issue:** https://github.com/ZenithResearch/hub/issues/48
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p21-004-ordinary-execution-path.md`

## Full spec

### Objective

Execute ordinary Matrix-originated work through Hub queue/case handlers without wallet/secS dependencies.
...
- ISS-P21-001, ISS-P21-002, PRP-PR-018

### Target files and surfaces

- Hub runtime/Frank/cases/queue integration, ingest fixtures, outbound Matrix adapter

### Locked decisions and invariants

- V0 executes ordinary Matrix-triggered work only.
- Mention/concierge routing must preserve Matrix provenance and bounded context.
- Passive rooms must not become queue/case work by default.

### Acceptance criteria

- Mention/concierge events enqueue bounded work with Matrix provenance.
- Passive events do not create cases.
- No wallet, secS-magik, or receipt-broker dependency is required for v0.
- Evidence is recorded in the implementation repo or linked capture before this issue is marked complete.

### Verification commands

- hub pytest; focused ingest/queue/cases tests; Matrix fixture smoke.
...
**Commit boundary:** one commit in the `ISS-P21-004` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P21-004`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p21-004"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p21-004 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p21-004"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p21-004 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p21-004 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p21-004 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p21-004`.
