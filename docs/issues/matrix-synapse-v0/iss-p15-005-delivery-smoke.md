# ISS-P15-005: Delivery smoke

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P15-005: Delivery smoke
- **Suggested branch:** `issue/iss-p15-005-delivery-smoke`
- **Suggested PR title:** `ISS-P15-005: Delivery smoke`
- **Primary repo:** `ZenithResearch/hub`
- **Supporting repo/API dependency:** none identified for this issue note
- **Source vault note:** `private source note: iss-p15-005`
- **GitHub issue:** https://github.com/ZenithResearch/hub/issues/39
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p15-005-delivery-smoke.md`

## Full spec

### Objective

Prove one feedback/concierge/mention delivery path reaches Hub queue/eventbus.
...
- ISS-P15-004, PRP-PR-018

### Target files and surfaces

- services/ingest/, services/queue, services/eventbus, scripts/smoke, tests
...
- Controlled Matrix event reaches intended Hub destination; event ID/correlation recorded; no private room content is over-ingested.
...
- pytest ingest/queue/eventbus; local fixture replay; optional prod smoke only with explicit approval.
...
**Commit boundary:** one commit in the `ISS-P15-005` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P15-005`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p15-005"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p15-005 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p15-005"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p15-005 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p15-005 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p15-005 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p15-005`.
