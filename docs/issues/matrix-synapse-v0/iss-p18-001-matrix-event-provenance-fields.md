# ISS-P18-001: Matrix event provenance fields

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P18-001: Matrix event provenance fields
- **Suggested branch:** `issue/iss-p18-001-matrix-event-provenance-fields`
- **Suggested PR title:** `ISS-P18-001: Matrix event provenance fields`
- **Primary repo:** `ZenithResearch/hub`
- **Supporting repo/API dependency:** none identified for this issue note
- **Source vault note:** `private source note: iss-p18-001`
- **GitHub issue:** https://github.com/ZenithResearch/hub/issues/40
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p18-001-matrix-event-provenance-fields.md`

## Full spec

### Objective

Define and persist provenance fields for Matrix-originated Hub events without treating Matrix identity as authority.
...
- PRP-PR-015 appservice readiness

### Target files and surfaces

- services/ingest/normalizer.py, queue/cases payloads, event schema/tests
...
- Events record platform, room_id, event_id, sender_id, timestamp, trigger type, correlation ID; authority fields remain separate.
...
- pytest ingest/cases/queue tests; schema/migration validation.
...
**Commit boundary:** one commit in the `ISS-P18-001` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P18-001`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p18-001"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p18-001 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p18-001"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p18-001 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p18-001 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p18-001 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p18-001`.
