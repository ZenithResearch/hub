# ISS-P18-003: Routing config model

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P18-003: Routing config model
- **Suggested branch:** `issue/iss-p18-003-routing-config-model`
- **Suggested PR title:** `ISS-P18-003: Routing config model`
- **Primary repo:** `ZenithResearch/hub`
- **Supporting repo/API dependency:** none identified for this issue note
- **Source vault note:** `private source note: iss-p18-003`
- **GitHub issue:** https://github.com/ZenithResearch/hub/issues/42
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p18-003-routing-config-model.md`

## Full spec

### Objective

Define operator-owned routing modes and allowed Matrix targets.
...
- config schema, libs/common/config.py, services/ingest, docs, tests
...
- Supports passive, mentions-only, concierge, agent-visible, authorized-action; missing/ambiguous route fails closed.
...
- pytest config/routing tests; config schema validation.
...
**Commit boundary:** one commit in the `ISS-P18-003` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P18-003`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p18-003"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p18-003 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p18-003"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p18-003 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p18-003 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p18-003 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p18-003`.
