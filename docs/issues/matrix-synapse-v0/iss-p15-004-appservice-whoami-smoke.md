# ISS-P15-004: Appservice whoami smoke

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P15-004: Appservice whoami smoke
- **Suggested branch:** `issue/iss-p15-004-appservice-whoami-smoke`
- **Suggested PR title:** `ISS-P15-004: Appservice whoami smoke`
- **Primary repo:** `ZenithResearch/hub`
- **Supporting repo/API dependency:** none identified for this issue note
- **Source vault note:** `private source note: iss-p15-004`
- **GitHub issue:** https://github.com/ZenithResearch/hub/issues/38
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p15-004-appservice-whoami-smoke.md`

## Full spec

### Objective

Add safe smoke commands/scripts for `/account/whoami` for every configured appservice.
...
- ISS-P15-002, ISS-P15-003

### Target files and surfaces

- scripts/, docs/operations, services/matrix_bridge tests
...
- Smoke asserts expected appservice identity and classifies network/TLS/auth/config failures; output redacts bearer tokens.
...
- smoke script with operator-provided env; curl whoami redacted; pytest.
...
**Commit boundary:** one commit in the `ISS-P15-004` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P15-004`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p15-004"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p15-004 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p15-004"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p15-004 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p15-004 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p15-004 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p15-004`.
