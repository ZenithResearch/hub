# ISS-P18-004: Mention-to-queue smoke

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P18-004: Mention-to-queue smoke
- **Suggested branch:** `issue/iss-p18-004-mention-to-queue-smoke`
- **Suggested PR title:** `ISS-P18-004: Mention-to-queue smoke`
- **Primary repo:** `ZenithResearch/hub`
- **Supporting repo/API dependency:** none identified for this issue note
- **Source vault note:** `private source note: iss-p18-004`
- **GitHub issue:** https://github.com/ZenithResearch/hub/issues/43
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p18-004-mention-to-queue-smoke.md`

## Full spec

### Objective

Verify a Matrix mention/concierge event becomes exactly one bounded Hub queue item with provenance.
...
- ISS-P18-001, ISS-P18-003

### Target files and surfaces

- services/ingest/appservice.py, normalizer.py, queue tests, smoke fixtures
...
- Controlled fixture produces one queue item; includes Matrix provenance; duplicate event IDs are safe/idempotent.
...
- pytest ingest/queue; fixture replay smoke.
...
**Commit boundary:** one commit in the `ISS-P18-004` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P18-004`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p18-004"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p18-004 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p18-004"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p18-004 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p18-004 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p18-004 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p18-004`.
