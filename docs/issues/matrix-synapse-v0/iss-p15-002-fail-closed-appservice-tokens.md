# ISS-P15-002: Fail-closed appservice tokens

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P15-002: Fail-closed appservice tokens
- **Suggested branch:** `issue/iss-p15-002-fail-closed-appservice-tokens`
- **Suggested PR title:** `ISS-P15-002: Fail-closed appservice tokens`
- **Primary repo:** `ZenithResearch/hub`
- **Supporting repo/API dependency:** none identified for this issue note
- **Source vault note:** `private source note: iss-p15-002`
- **GitHub issue:** https://github.com/ZenithResearch/hub/issues/36
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p15-002-fail-closed-appservice-tokens.md`

## Full spec

### Objective

Require Gateway/bridge/Sophia appservice tokens outside explicit local-dev mode.
...
- services/ingest/, services/matrix_bridge/, libs/common/config.py, tests
...
- Missing/empty token fails explicitly outside local dev; logs name missing config without values; negative tests pass.
...
- pytest missing-token tests; env-unset smoke; private artifact scan.
...
**Commit boundary:** one commit in the `ISS-P15-002` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P15-002`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p15-002"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p15-002 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p15-002"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p15-002 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p15-002 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p15-002 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p15-002`.
