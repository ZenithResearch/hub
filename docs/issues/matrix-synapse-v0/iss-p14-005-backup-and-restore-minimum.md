# ISS-P14-005: Backup and restore minimum

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P14-005: Backup and restore minimum
- **Suggested branch:** `issue/iss-p14-005-backup-and-restore-minimum`
- **Suggested PR title:** `ISS-P14-005: Backup and restore minimum`
- **Primary repo:** `ZenithResearch/hub`
- **Supporting repo/API dependency:** none identified for this issue note
- **Source vault note:** `private source note: iss-p14-005`
- **GitHub issue:** https://github.com/ZenithResearch/hub/issues/33
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p14-005-backup-and-restore-minimum.md`

## Full spec

### Objective

Add backup/snapshot policy and restore documentation for Synapse DB, media store, signing key, config, and appservice tokens.
...
- infra/aws_baseline_80/backup or EBS snapshot resources, docs/operations, infra/matrix docs
...
- Backup target/schedule/retention/owner and restore path are explicit; untested restore paths are marked unproven; no data-safety overclaim.
...
- terraform plan; backup resource listing; restore runbook review; optional non-prod restore dry run.
...
**Commit boundary:** one commit in the `ISS-P14-005` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P14-005`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p14-005"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p14-005 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p14-005"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p14-005 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p14-005 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p14-005 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p14-005`.
