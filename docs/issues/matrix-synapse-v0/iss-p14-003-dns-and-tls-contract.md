# ISS-P14-003: DNS and TLS contract

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P14-003: DNS and TLS contract
- **Suggested branch:** `issue/iss-p14-003-dns-and-tls-contract`
- **Suggested PR title:** `ISS-P14-003: DNS and TLS contract`
- **Primary repo:** `ZenithResearch/hub`
- **Supporting repo/API dependency:** none identified for this issue note
- **Source vault note:** `private source note: iss-p14-003`
- **GitHub issue:** https://github.com/ZenithResearch/hub/issues/31
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p14-003-dns-and-tls-contract.md`

## Full spec

### Objective

Add or specify Route53/TLS handling for `synapse.zenith-research.ca`, including client API and federation posture.
...
- infra/aws_baseline_80/dns.tf, alb/proxy/TLS resources or Synapse host user-data, docs
...
- DNS record and TLS termination owner are explicit; client API is HTTPS; federation 8448 is open only if intentionally enabled.
...
- dig synapse.zenith-research.ca; openssl s_client; curl Matrix client versions/health; terraform plan.
...
**Commit boundary:** one commit in the `ISS-P14-003` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P14-003`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p14-003"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p14-003 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p14-003"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p14-003 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p14-003 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p14-003 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p14-003`.
...
- Issue PR branch: `issue/iss-p14-003-dns-tls-contract-main`
- Targeted test: `uv run --with pytest pytest tests/matrix/test_iss_p14_003_dns_tls.py -q`
- Scope: this PR completes ISS-P14-003: DNS and TLS contract only; it does not claim production deployment, appservice delivery, or Matrix identity as Hub authority.
