# ISS-P14-006: Static deployment-path tests

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P14-006: Static deployment-path tests
- **Suggested branch:** `issue/iss-p14-006-static-deployment-path-tests`
- **Suggested PR title:** `ISS-P14-006: Static deployment-path tests`
- **Primary repo:** `ZenithResearch/hub`
- **Supporting repo/API dependency:** none identified for this issue note
- **Source vault note:** `private source note: iss-p14-006`
- **GitHub issue:** https://github.com/ZenithResearch/hub/issues/34
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p14-006-static-deployment-path-tests.md`

## Full spec

### Objective

Add checks proving Synapse resources live in `infra/aws_baseline_80` and generated appservice files remain runtime-only.
...
- tests/, scripts/matrix_deployment_check.py, CI/private-artifact scan scripts
...
- Tests fail if Synapse is only in separate module; generated appservice files are not tracked; static checks require no production mutation.
...
- python tests; terraform init -backend=false && validate; private artifact scan.
...
**Commit boundary:** one commit in the `ISS-P14-006` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P14-006`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p14-006"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p14-006 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p14-006"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p14-006 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p14-006 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p14-006 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p14-006`.
